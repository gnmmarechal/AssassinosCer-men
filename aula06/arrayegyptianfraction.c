...
PtEgyptianFraction EgyptianFractionCreate (PtFraction pfraction)	/* construtor inicializador - initializer constructor */
{
	...
	PtFraction Frac = FractionCreate (FractionGetNumerator(pfraction), FractionGetDenominator(pfraction));
	if(Frac == NULL){
		Error = NO_MEM;
		return NULL;
	}
	...
	*/
	for(int i = 0; i < MAX_SIZE; i++){
		Egipto -> Array[i] = CreateUnitFraction(Frac);
		Egipto -> Size = Egipto -> Size + 1;
		if(Egipto -> Array[i] == NULL){
			Egipto -> Complete = 1;
			Error = OK;
			FractionDestroy(Frac);
			return Egipto;
		}
		Frac = FractionSubtraction(Frac, Egipto -> Array[i]);
	}
	if(FractionGetDenominator(Frac) == 0){
		Error = OK;
		Egipto -> Complete = 1;
	}
	Error = OK;
...
PtFraction EgyptianFractionToFraction (PtEgyptianFraction pegyp)
{
	...
	for(int i = 1; i < EgyptianFractionGetSize; i++){
...
int EgyptianFractionEquals (PtEgyptianFraction pegy1, PtEgyptianFraction pegy2)
{
	...
	Error = OK;
	if(EgyptianFractionGetSize(pegy1) != EgyptianFractionGetSize(pegy2))
		return 0;
	for(int i = 0; i < EgyptianFractionGetSize(pegy1); i++){
		if(FractionEquals(pegy1,pegy2) == 0)
			return 0;
	}	
	return 1;
}
